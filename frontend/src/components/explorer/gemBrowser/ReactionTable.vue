    ...
    <table class="table is-bordered is-striped is-narrow is-fullwidth" ref="table">
      ...
      <tbody>
        <tr v-for="(r, index) in sortedReactions">
          ...
          <td>
            <template v-for="(m, index) in r.modifiers">{{ index == 0 ? '' : ', '}}<router-link :to="{ path: `/explore/gem-browser/${model}/enzyme/${m.id}` }">{{ m.name }}</router-link>
            </template
          </td>
          ...
          <td v-show="showSubsystem">
            <template v-if="r.subsystem">
              ...
              {{ index == 0 ? '' : '; '}}<router-link :to="{ path: `/explore/gem-browser/${model}/subsystem/${idfy(s)}` }">{{ s }}</router-link></a>
              </template>
            </template>
          </td>

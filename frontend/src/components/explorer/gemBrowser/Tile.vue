  ...
  <div class="tile is-parent" :class="size" @click="navigate()">
    <div class="tile is-child box">
      ...
      <template v-if="type == 'metabolite'">
        <span class="is-capitalized"><b>{{ data.name }}</b></span> with {{ model }} ID <b>{{ data.id}}</b> is in the <b>{{ data.compartment }}</b> compartment.
      ...
      <template v-else>
        <b>{{ data.name}}</b> with {{ model }} ID <b>{{ data.id}}</b> catalyzes <b>{{ data.reaction_count }}</b> reaction(s) across <b>{{ data.subsystem_count }}</b> subsystem(s) and <b>{{ data.compartment_count }}</b> compartment(s).
      </template>
      <slot></slot>
    </div>
  </div>

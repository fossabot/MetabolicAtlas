app.controller('v4ElemsCtrl', [ '$scope', '$http', 'view4Graph', function( $scope, $http, view4Graph ){
  ...
  $scope.API = mainURL+"enzymes/ENSG00000180011/connected_metabolites?include_expressions=true";

  $scope.dosearch = function(fsearch) {
    $scope.search = angular.copy(fsearch);
    //$scope.API = mainURL+"reaction_components/E_2571/interaction_partners";
    $scope.API = mainURL+"enzymes/"+$scope.search.rc+"/connected_metabolites?include_expressions=true";
